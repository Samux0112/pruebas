    ...
    btnAccion.addEventListener('click', function () {
        ...
        if (filas < 2) {
            ...
            http.send(JSON.stringify({
                ...
                bSalida: bodegaSalida.value,
				bEntrada : bodegaEntrada.value,
                        ...
                        setTimeout(() => {
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    const ruta = base_url + 'salidas/reporte/impresion/' + res.idCotizacion;
...
function verReporte(idCotizacion) {
    Swal.fire({
        ...
        /* Read more about isConfirmed, isDenied below */
        if (result.isConfirmed) {
            const ruta = base_url + 'salidas/reporte/impresion/' + idCotizacion;
            window.open(ruta, '_blank');
        } else if (result.isDenied) {
            const ruta = base_url + 'salidas/reporte/impresion/' + idCotizacion;
            window.open(ruta, '_blank');
