...
$sql = "select * from dtes where id=(".$idFactura.")";
...
if($dteStr["dteJson"]["identificacion"]["tipoDte"]!="01"){
...
<script>
   ...
   var iva = "<?php if($dteStr["dteJson"]["identificacion"]["tipoDte"]=="01"){echo 0;}else{doubleval($iva);} ?>";
   var tipo = "<?php echo  $dteStr["dteJson"]["identificacion"]["tipoDte"] ?>" == "01" ? "Consumidor Final" : "Credito Fiscal";
   iva = parseFloat(iva);
    if(mes<10){
	  mes = "0"+mes;
  }
  ...
  var jsondteObj ={
   ...
   dteJson: {
      ...
      documento: {
         ...
         tipoDocumento: tipo != "Consumidor Final" ? "36" : numdocumento > 10 ? "36" : "13",
         numDocumento: tipo != "Consumidor Final" ? numdocumento : numdocumento ,
         nombre: "<?php echo  $dteStr["dteJson"]["receptor"]["nombre"] ?>"
      },
      motivo: {
  ...
  var jsondteObj ={
   ...
   dteJson: {
      ...
      documento: {
         ...
         tipoDocumento: tipo != "Consumidor Final" ? "36" : "36",
         numDocumento: tipo != "Consumidor Final" ? numdocumento : numdocumento ,
         nombre: "<?php echo  $dteStr["dteJson"]["receptor"]["nombre"] ?>"
      },
      motivo: {
...
xhttp.onreadystatechange = function() {
  ...
  if (this.readyState == 4 && this.status == 200) {
	...
	alert(jsonResponse.descripcionMsg);
	 $('#load').modal('hide'); 
	         const url = base_url + 'ventas/anular/' + <?php echo $idFactura; ?>;
